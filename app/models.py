...
class User(db.Model):
    ...
    customs_records = db.relationship('CustomsRecord', backref='user', lazy=True, cascade='all, delete-orphan')
...
class Document(db.Model):
    ...
    type = db.Column(db.Enum('passport', 'id_card', name='document_types'), nullable=False)
    document_number = db.Column(db.String(50), nullable=False)
    expiration_date = db.Column(db.Date, nullable=False)
    file_url = db.Column(db.String(255), nullable=False)
    status = db.Column(db.Enum('pending', 'approved', 'rejected', name='document_status'), 
...
class CustomsRecord(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=False)
    status = db.Column(db.Enum('activo', 'inactivo', 'pendiente', name='customs_status'), 
