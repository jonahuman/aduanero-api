...
@documents_bp.route('/<document_id>/review', methods=['PUT'])
@jwt_required()
def review_document(document_id):
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Error al revisar documento'}), 500
...
def delete_document(document_id):
    try:
        ...
        if document.user_id != current_user_id and not current_user.is_admin:
            return jsonify({'error': 'Permisos insuficientes'}), 403
